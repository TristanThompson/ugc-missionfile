...
*/ 
private["_kassa","_ui","_progress","_pgText","_cP","_rip","_alarm","_shopName","_bankpos"];
params[
	["_shop",ObjNull,[ObjNull]],
	["_robber",ObjNull,[ObjNull]],
	"_action"
];
_shopName = "MorrisonTown Bank";
...
if(_rip) then
{
    ...
    if!(alive _robber) exitWith { 
		...
		[1," Morrison Town Bank: Der Täter ist verstorben!"] remoteExec ["life_fnc_broadcast",west];
		_rip = false;
		5 cutText ["","PLAIN"];
	};
	if(life_istazed) exitwith {
		...
		5 cutText ["","PLAIN"];
		[1," Morrison Town Bank: Der Täter wurde erfolgreich gestellt!"] remoteExec ["life_fnc_broadcast",west];
    ...
    if(_robber distance _shop > 3) exitWith { 
		...
		5 cutText ["","PLAIN"];
		[1," Morrison Town Bank: Der Täter ist ohne Geld geflüchtet!"] remoteExec ["life_fnc_broadcast",west];
		_rip = false;
		deleteMarker "Marker200";
	};
