...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	[[1,2],localize "STR_ISTR_Bolt_AlertFed"] remoteExec ["life_fnc_broadcast",-2];
} else {
	...
	[[1,2],format[localize "STR_House_Break_NOTF",(_building getVariable "house_owner") select 1]] remoteExec ["life_fnc_broadcast",-2];
};
...
while {true} do
{
	if(animationState player != "cl3_anim_drill_short") then {
		[player,"cl3_anim_drill_short"] remoteExec ["life_fnc_animSync",-2];
