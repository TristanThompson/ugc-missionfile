...
while {true} do
{
	...
	while {true} do
	{
		...
		if(v4r14t0X_isTc00L < _fuel_price_one_step) exitWith {hint format["Du brauchst mehr Geld um weiter zu Tanken. Voller Tank kostet $%1",_fuel_price]; sleep 1;};

		_fuel_old = fuel _veh;
		_fuel_new = _fuel_old + (1 / _steps);
		_veh setFuel _fuel_new;
		
		v4r14t0X_isTc00L = v4r14t0X_isTc00L - _fuel_price_one_step;
